...
function initScrub(cfg) {
 ...
 // cfg.defer: don't fetch this section's frames until the user is within
 // two viewports of it (saves ~14MB on initial load per deferred section).
 if (cfg.defer && "IntersectionObserver" in window) {
  new IntersectionObserver((entries, io) => {
   if (entries[0].isIntersecting) { loadFrames(); io.disconnect(); }
